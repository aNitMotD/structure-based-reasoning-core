...
This directory contains non-prescriptive observation logs of CIF-related closure patterns.
Each file records a real-world sequence of events without evaluation, recommendation, or attribution of fault.

### Scope

Logs in this directory may range from single-interaction observations
to higher-level phenomena observed across multiple models or sessions.
...
- `confirmed/`  
  Cases where Constraint Inheritance Failure (CIF)
  has been structurally observed and closed.

- `pre-closure/`  
  Observations recorded along CIF-like structural paths
...
이 디렉토리는 CIF와 관련된 판단 종결 패턴을  
평가·권고·책임 귀속 없이 관측 기록 형태로 보관한다.  
각 로그는 실제 사건의 전이 과정을 그대로 기록한다.

### 범위

이 디렉토리의 로그는 단일 상호작용 관측부터  
다중 모델·다중 세션에 걸친 현상 관측까지 포함할 수 있다.
...
- `confirmed/`  
  Constraint Inheritance Failure(CIF)가  
  구조적으로 관측되고 판정이 닫힌 사례들.

- `pre-closure/`  
  CIF와 유사한 구조 경로 상에서 관측되었으나,  
